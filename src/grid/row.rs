//! Row type for terminal grid lines
...
use super::cell::{Cell, ResetDiscriminant};

/// A row in the terminal grid
#[derive(Serialize, Deserialize, Default, Clone, Debug)]
pub struct Row {
    inner: Vec<Cell>,
    /// Maximum number of occupied entries since last reset
    pub occ: usize,
}
...
impl Row {
    /// Create a new terminal row with specified number of columns
    pub fn new(cols: usize) -> Row {
        debug_assert!(cols >= 1);

        let mut inner: Vec<Cell> = Vec::with_capacity(cols);

        // Optimized version of std::vec::Vec::resize
        unsafe {
    ...
    /// Returns removed non-empty cells if any
    ...
    /// Returns true if no cells contain an end of prompt marker
    #[inline]
    pub fn has_no_end_of_prompt_marker(&self) -> bool {
        self.inner.iter().all(|cell| !cell.is_end_of_prompt())
    }
